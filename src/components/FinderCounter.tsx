...
export default function FinderCounter({ label, count: countProp, updatedAt: updatedAtProp }: { label?: string; count?: number | null; updatedAt?: string | null } = {}) {
  ...
  return (
    <div className="animate-fade-up flex flex-col items-center mb-2 sm:mb-6">
      <button
        type="button"
        onClick={handleSplash}
        onMouseEnter={handleHover}
        className="relative inline-flex items-center gap-1 sm:gap-2.5 rounded-full px-2.5 py-1 sm:px-5 sm:py-2.5 shadow-lg cursor-pointer transition-transform hover:scale-[1.03] active:scale-95"
        style={{
      ...
      >
        ...
        <span className="font-body text-[11px] sm:text-base" style={{ color: "rgba(245,230,211,0.9)" }}>
          {label ?? "Енотыча нашли"}
        </span>
        <span
          className="font-display font-bold text-[13px] sm:text-xl tabular-nums"
      ...
      {updatedLabel && (
        <p className="font-body text-[9px] sm:text-xs mt-1 sm:mt-2" style={{ color: "rgba(245,230,211,0.55)" }}>
